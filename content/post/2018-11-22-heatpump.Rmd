...
ggplot(hydro %>% filter(date>=  lubridate::ymd(start) ))+
  ...
  labs(title = wrapper("Hourly electricty consumption appears linearly related to exterior temperature",50))
...
We will then be able to calculate the coefficient of performance the heat pump would have had for each year.
...
First, a sanity check: the model predicts that I should spend about 1708$ per year on average on heating if I do not change anything.  This is 57% of my total electricity bill, which fits right inside the  rule of thumb we mentionned earlier.
...
Ignoring compounding interests (it is 2AM after all), the payback period on the extra 4000 \$ I spend to get the heat pump would only be 5 years.  The remaining 5 years of life in the heat pump would be profit, or a cushion to protect against problems.  
